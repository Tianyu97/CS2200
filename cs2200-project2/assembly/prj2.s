...
main:	lea $sp, initsp                         ! initialize the stack pointer
        ...
        lea     $t1, garden_handler             ! Install keyboard interrupt handler into vector table
        sw      $t1, 1($t0)
...
timer_handler:
        addi    $sp, $sp, -1                    ! save $k0 to the stack
        sw      $k0, 0($sp)

        ei                                      ! enable interrupts
        addi    $sp, $sp, -13                   ! save ALL registers to the stack
        sw      $ra, 12($sp)
        sw      $fp, 11($sp)
        sw      $s2, 10($sp)
        sw      $s1, 9($sp)
        ...
        ! execute device code
        lea     $t0, ticks                      ! t0 <- addr of TICKS
        lw      $t1, 0($t0)                     ! t1 <- MEM[addr of TICKS] (0xFFFF)
        lw      $t2, 0($t1)                     ! t2 <- MEM[0xFFFF]
        addi    $t2, $t2, 1                     ! increment t2
        sw      $t2, 0($t1)                     ! MEM[0xFFFF] <- $t1

        lw      $ra, 12($sp)                    ! restore ALL registers from the stack
        lw      $fp, 11($sp)
        lw      $s2, 10($sp)
        lw      $s1, 9($sp)
        lw      $s0, 8($sp)
...
garden_handler:
        ...
        ! execute device code
        lea     $t0, maxval                     ! t0 <- addr of maxval
        lw      $t1, 0($t0)                     ! t1 <- MEM[addr of maxval] (0xFFE0)
        lw      $t2, 0($t1)                     ! t2 <- MEM[0xFFE0] = current max val
        in      $t0, 1                          ! get reading from I/O device
        bgt     $t2, $t0, restore               ! branch right to restore if current max is greater
        sw      $t0, 0($t1)                     ! otherwise, write the new max to memory
